...
void prueba_desapilar_elementos(void){
    ...
    print_test("Se desapilo el ultimo elemento exitosamente", pila_desapilar(pila) == &valores[0]);
    print_test("Desapilar con la pila vacia devuelve NULL", pila_desapilar(pila) == NULL);
    apilar_varios(pila, pila_apilar, "Apilar varios elementos funciona bien despues de usar desapilar");
    pila_destruir(pila);
}
